...
namespace Life.Application.GameSession.Contracts
{
    ...
    public static class GameSessionMappingExtensions
    {
        ...
        // TODO: Fix this extension method to work with current domain model
        // public static Life.Domain.Aggregates.Board ToBoard(this CreateFromPatternRequest request)
        // {
        //     var dimensions = new BoardDimensions(request.Width, request.Height);
        //     var center = request.CenterPosition?.ToPosition() ?? new Position(request.Width / 2, request.Height / 2);
        //     var patternCells = GameSessionService.GeneratePatternCells(request.Pattern, center);
        //     return new Life.Domain.Aggregates.Board(dimensions, patternCells);
        // }
        ...
        public static SessionStatisticsResponse ToResponse(this SessionStatistics stats)
        {
            return new SessionStatisticsResponse
            {
                ...
                LivingCells = stats.CurrentPopulation, // Use current population as living cells
                DeadCells = 0, // Not tracked in current stats model
                PopulationDensity = 0.0, // Would need calculation
                BirthsThisGeneration = 0, // Not tracked in current stats model
                DeathsThisGeneration = 0, // Not tracked in current stats model
                TotalRuntime = stats.SessionDuration, // Use session duration
        ...
        {
            // TODO: Deserialize BoardData to get actual board state
            var boardResponse = new BoardResponse
            {
                Generation = snapshot.Generation,
                Dimensions = new DimensionsResponse { Width = 10, Height = 10 }, // Placeholder
                LivingCells = new List<CellResponse>() // Empty for now
            };
            
            return new SnapshotResponse
            {
                Id = snapshot.Id,
                Name = snapshot.Description, // Use description as name
                Board = boardResponse,
